# Draft — Pricing/Warranty/FAQ sections for the 3 gap pages

**Company:** Amfa Furniture Ltd
**Channel:** Website (live preview `amfa.tilda.ws`) — filling the structural gap identified in
`Research/2026-09-21_Amfa_Live-Site_Pre-Launch-Review.md` §3c: Living & Media Furniture, Custom Storage
Solutions, and Commercial Furniture are the only 3 of 8 category pages without a pricing/warranty/FAQ
section.
**Brief:** Minda, 2026-09-21 — "let's work on pricing/warranty/FAQ section".
**Status:** Final. All 3 sections confirmed by Minda, 2026-09-21. Living & Media and Custom Storage use
"4–8 weeks" / "from £1,500". Commercial Furniture is priced individually per project, no published
floor. The project-deadline FAQ question was dropped rather than answered (Minda, 2026-09-21) — no
open items remain.

---

## What's already confirmed and safely reused (not invented)

The warranty wording is **identical, word-for-word, across all 5 existing category pages** — it's
clearly a standing company policy, not something specific to each category, so reusing it verbatim for
these 3 pages isn't inventing anything:

> *"We provide a warranty of up to 10 years on the furniture we manufacture and the hardware used
> (depending on the materials and manufacturers selected). The warranty covers manufacturing defects
> and workmanship quality, provided the furniture is used and maintained correctly."*

FAQ content below draws only on facts already confirmed elsewhere on the site or in this KB (the
"Residential & commercial" differentiator, the standard consultation → design → manufacture →
install process, the free-quote model) — nothing category-specific is invented.

---

## 1. Living & Media Furniture

### Manufacturing time, pricing and warranty

The average manufacturing time for furniture is **4–8 weeks** after the design and materials have
been approved. Production time depends on:
- The complexity of the project
- The materials selected
- Non-standard sizes
- Custom finishes (media wall integrations, LED lighting, electric fireplace surrounds)
- The availability of special order components

The cost is calculated individually and depends on factors such as:
**Prices from £1,500**
- The size and scope of the piece (a single TV unit vs. a full media wall composition)
- The materials selected
- The finish options
- The number of interior elements
- The complexity of the design

We provide a warranty of up to 10 years on the furniture we manufacture and the hardware used
(depending on the materials and manufacturers selected). The warranty covers manufacturing defects and
workmanship quality, provided the furniture is used and maintained correctly.

### FAQ

**Can you build a media wall around an existing TV and sound system?**
Yes. We plan the layout around your equipment, sockets, cable management, and ventilation requirements
as part of the design stage.

**Do you provide installation?**
Yes. We offer a complete service, from design and manufacturing through to professional installation.

**Can I order a single TV unit rather than a full media wall?**
Yes. If a full media wall isn't required, we can manufacture a custom TV unit in any configuration —
wall-mounted, freestanding, or a long modern console.

**How long does manufacturing take?**
On average, production takes **4–8 weeks** after final design approval.

**Do you provide a warranty?**
Yes. All furniture comes with a warranty covering workmanship and materials, in accordance with the
company's warranty terms.

---

## 2. Custom Storage Solutions

### Manufacturing time, pricing and warranty

The average manufacturing time is **4–8 weeks** after the design and materials have been approved.
Production time depends on:
- The complexity and scale of the project
- The materials selected
- Non-standard sizes or an unusual space (alcoves, sloped ceilings, awkward corners)
- The availability of special order components

The cost is calculated individually and depends on factors such as:
**Prices from £1,500**
- The size and scope of the storage solution
- The materials selected
- The complexity of the space being fitted
- The interior fittings and finish options

We provide a warranty of up to 10 years on the furniture we manufacture and the hardware used
(depending on the materials and manufacturers selected). The warranty covers manufacturing defects and
workmanship quality, provided the furniture is used and maintained correctly.

### FAQ

**What kind of spaces can you design storage for?**
Any space that doesn't fit neatly into a standard kitchen, bedroom, or living room category —
alcoves, awkward corners, understairs areas, sloped ceilings, and other non-standard architectural
features.

**Is every project fully bespoke?**
Yes. Every custom storage project is developed from scratch, based on the room's dimensions,
architectural features, intended use, and your storage requirements.

**Do you provide installation?**
Yes. We offer a complete service, from design and manufacturing through to professional installation.

**How long does manufacturing take?**
On average, production takes **4–8 weeks** after final design approval.

**Do you provide a warranty?**
Yes. All furniture comes with a warranty covering workmanship and materials, in accordance with the
company's warranty terms.

---

## 3. Commercial Furniture

**Pricing approach confirmed by Minda, 2026-09-21: priced individually per project, no published
starting price.**

### Pricing, manufacturing time and warranty

Commercial furniture is priced individually for every project — from a single reception desk to a
full hotel fit-out, costs depend on scope, scale, and site requirements. Get in touch for a free
consultation and quote.

Manufacturing and installation timescales are agreed individually for each commercial project, taking
into account the scope of work, site access, and any deadlines tied to a wider fit-out or opening date.

We provide a warranty of up to 10 years on the furniture we manufacture and the hardware used
(depending on the materials and manufacturers selected). The warranty covers manufacturing defects and
workmanship quality, provided the furniture is used and maintained correctly.

### FAQ

**Do you work with businesses as well as private homes?**
Yes. We work with offices, hotels, retail spaces, serviced apartments, property management companies,
developers, and other commercial and investment properties.

**Do you provide installation?**
Yes. We offer a complete service, from concept development through to professional installation.

**Do you provide a warranty?**
Yes. All furniture comes with a warranty covering workmanship and materials, in accordance with the
company's warranty terms.

---

## Sources

- `amfa.tilda.ws` — the 5 existing category pages (Kitchens, Wardrobe & Bedroom, Bathroom, Home
  Office, Hallway & Utility), read directly for the exact warranty wording and the structural pattern
  being matched here.
- `Brand-and-Voice/Amfa-Furniture-Ltd.md` — the "Residential & commercial" differentiator and general
  process facts used in the FAQ content.
- `Research/2026-09-21_Amfa_Live-Site_Pre-Launch-Review.md` — where this gap was first identified.
- Minda, in chat, 2026-09-21 — confirmed the Living & Media / Custom Storage manufacturing-time and
  pricing figures ("Prepose accepted"), converting them from a proposed placeholder to a fact; chose
  the priced-per-project approach for Commercial Furniture over a published starting price; and said
  to drop the project-deadline FAQ question rather than answer it.
